...
Param
    ...
    {
        # A descriptive error!
        Write-Output "Erm...Storage account doesn't exist check for typos" -ForegroundColor Red
    }
...
Param
    ...
    Catch
    {
        Write-Output "Erm...container doesn't exist check for typos" -ForegroundColor Red
...
Param
    ...
    Catch
    {
        Write-Output "Erm...blob doesn't exist check for typos" -ForegroundColor Red
...
Function ConnectToExchange {
    ...
    Catch
    {
        ...
        Write-Output "Invalid credentials, please re-enter your credentials." -ForegroundColor Yellow
...
Function CheckDomain {
    ...
    $sDomainName
		...
		if ($sDomainName -notmatch "^(?!www\.)(?!\.)([a-z0-9\-\.]+)+((com)|(me\.uk)|(co\.uk)|(eu)|(org)|(ac))$"){
            Write-Output $sDomainName "does not match the expected format for a domain name e.g. site.com|.co.uk|.eu|.org | .ac is valid" -ForegroundColor Red
            $Validator = $false
        }
		else {
...
Function WriteToLog {
	...
	If (Test-Path (Join-Path -Path (Get-Location) -ChildPath $sLogFile)) {
	
		# Append to the file
		Add-Content -Path (Join-Path -Path (Get-Location) -ChildPath $sLogFile) -Value $sLogContent
		
	}
	Else {
	
		# Create the file
		New-Item -ItemType File -Path (Join-Path -Path (Get-Location) -ChildPath $sLogFile)
		Add-Content -Path (Join-Path -Path (Get-Location) -ChildPath $sLogFile) -Value $sLogContent
...
Function CheckDirectory {
    ...
    If (-not (Test-Path $sFolderPath)) {
        ...
        New-Item -ItemType Folder $sFolderPath
...
Function Load_ActiveDirectoryTools
{
	...
	$sVer = ReadReg "Hklm:\Software\Microsoft\Windows NT\CurrentVersion\" "CurrentVersion"
		...
		switch ($sVer)
		{
			Default
			{
				Write-Output "Unsupported operating system version" -ForegroundColor Red
			}
			"6.0"
			{
				while (!$Valid)
				{
					Add-PSSnapin WebAdministration
						if (!$?) {
				  			Write-Output "Windows 2008 detected, but no PS Snapin Available - Download from: http://www.iis.net/download/PowerShell" -ForegroundColor Red
			...
			"6.1"
			{
				while (!$Valid)
				{
					Import-Module WebAdministration
						if (!$?) {
                			Write-Output "Windows R2 detected, but no WebAdministration Module Available - Install via Server Manager" -ForegroundColor Red
